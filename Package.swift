...
// Architecture & module boundaries: docs/architecture/04-kien-truc.md
...
let package = Package(
    ...
    dependencies: [
        ...
        // Redis/Valkey driver (docs/architecture/15, KV-*). RediStack (the design
        ...
        // minimum stays .macOS(.v14) (docs/architecture/15 §3).
        ...
        // `ssh-rsa` by default (KN-03). Re-point at upstream once that PR
        ...
        // Sparkle auto-update (docs/architecture/10 §3) is OPT-IN for release
        // builds only: it adds an embedded binary framework, which the
        // dependency-free default build + size guard (08 §6) must not carry.
    ...
    targets: [
        // MARK: Driver contracts (docs/architecture/05) — no dependencies
        .target(
            name: "BerryDriverKit",
        ...
        // MARK: NoSQL/vector contracts (docs/architecture/12 §2) — sibling of
        ...
        // MARK: Key-value contracts (docs/architecture/15 §2) — third driver
        ...
        // MARK: Qdrant driver (docs/architecture/12 §5) — zero vendored deps,
        // plain REST/JSON over URLSession (same pattern as the AI client, 03 §2).
        .target(
        ...
        // MARK: Elasticsearch driver (docs/architecture/17) — zero vendored
        ...
        // MARK: MongoDB driver (docs/architecture/12 §3) — zero vendored deps,
        ...
        // project has been in "Development Pause" (EOL, no further commits)
        // since 2023 — see 12 §3 "Trạng thái hiện thực" for the full story.
        .target(
        ...
        // MARK: DynamoDB driver (docs/architecture/12 §4) — PartiQL over the
        // existing DatabaseDriver contract. Zero vendored dependency: hand-rolled
        // AWS SigV4 (CryptoKit, an Apple platform framework already used
        // elsewhere in the app — not a new SwiftPM package) + plain REST/JSON
        // over URLSession, same shape as BerryDriverQdrant. `aws-sdk-swift`
        // (the doc's stated preference) was tried first and abandoned — see
        // 12 §4 "Trạng thái hiện thực" for why.
        .target(
        ...
        // MARK: SQL Server driver (docs/architecture/05 §4, V2⚠️) — the one
        ...
        // rejected — see docs/architecture/16-sql-server.md). Bridges to
        // FreeTDS's DB-Library C API (`-lsybdb`), LGPL, must stay dynamically
        // linked (NOTICE requirement, docs/architecture/10 §3). Homebrew's
        ...
        .target(
            ...
            cSettings: [.unsafeFlags(["-I/opt/homebrew/opt/freetds/include"])],
            swiftSettings: [.unsafeFlags(["-Xcc", "-I/opt/homebrew/opt/freetds/include"])],
            linkerSettings: [
                .unsafeFlags(["-L/opt/homebrew/opt/freetds/lib"]),
                .linkedLibrary("sybdb"),
            ]
        ),

        // MARK: Redis/Valkey driver (docs/architecture/15, KV-*) — implements
        ...
        // MARK: SSH tunnel (KN-03) — local forwarder over Citadel
        .target(
            name: "BerryTunnel",
            dependencies: [
                "BerryDriverKit",
        ...
        // vector-search SQLite extension for local chat-message RAG (docs/agents/
        // architecture/11 §7.3). SQLITE_CORE links it directly against the
        // system libsqlite3 symbols instead of the loadable-extension API, so
        // it can be registered once via sqlite3_auto_extension (07 §7) without
        ...
        // MARK: Local persistence — profiles/history/prefs (docs/architecture/07 §3)
        .target(
            name: "BerryStore",
            dependencies: [
                "BerryDriverKit",
        ...
        // MARK: Licensing & commercialization client (docs/architecture/10)
        ...
        // MARK: Database Intelligence graph (docs/architecture/11) — in-memory
        ...
        // MARK: AI agent client (docs/architecture/09) — SSE consumer + tool
        ...
        // graph_query tool (11 §7). BerryStore: local chat history + sqlite-vec
        // RAG (Q17, docs/agents/architecture/11 §7) — already reachable
        ...
        // BerryGraph backs harvest-on-refresh + the graph_query tool (11 §4/§7).
        .target(
            name: "BerryUI",
            // BerryDataSourceKit: the Mongo/Qdrant sidebar/tab/grid path
            // (docs/architecture/12 §7) touches only the DataSourceDriver
            // protocol surface — never a concrete driver package, same
            // registry-pattern discipline as the SQL side. BerryKeyValueKit:
            // the Redis key-browser path (docs/architecture/15 §2) — same
        ...
        .testTarget(
            ...
            // BerryDriverSQLite: the Query Analyzer (DI-06) plans EXPLAIN output
        ...
        .testTarget(
            ...
            // (docs/architecture/12 §7) has its own Docker-gated conformance
            // suite here, same pattern as the SQLite one above.
            // BerryKeyValueKit/BerryDriverRedis: the Redis workspace wiring
            // (docs/architecture/15 §3) has its own Docker/local-server-gated
        ...
        // MARK: Performance benchmarks (M7, docs/architecture/01 §2) — opt-in
