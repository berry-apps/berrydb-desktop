...
/// non-SQL sibling of `DangerGuard` (BerryCore).
...
/// Deliberately separate from BerryCore's
...
public enum DataSourceDangerLevel: Equatable, Sendable {
 ...
 /// — the user must retype the exact
...
public enum DataSourceDangerReason: Equatable, Sendable {
 ...
 /// DELETE without WHERE: every point/document in
...
/// `updateMany` with `{}`, not just Qdrant.
public enum DataSourceDangerGuard {
 ...
 /// `.deleteByFilter`/`.updateByFilter`'s `filter`.
