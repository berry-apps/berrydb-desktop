...
/// Builds the DSG from a live connection's schema (docs/architecture/11 §4,
/// DI-01). **Metadata only** — reads objects + table detail through the
/// introspector (`SchemaCatalog`), never SELECTs user data (Q6). Harvesting is
/// off by default on production profiles (KN-07); pass `allowProduction` to
...
public enum SchemaHarvester {
    ...
    public static func buildGraph(
        ...
        for object in objects where object.kind == .table {
            ...
            // runs against a mutating DB (KN-07 background pass). Skip the
    ...
    public static func harvest(
        ...
        // Enrich with catalog/stats views (DI-05 foundation) — best-effort,
